...
def calculate_snr(dataset: Dataset, targets: Union[str, Sequence[str]] = 'subbytes', chunk_size: int = 1024):
    ...
    per_target_means = {key: np.zeros((256, base_dataset.timesteps_per_trace), dtype=np.float32) for key in targets}
    ...
    noise_variance = {key: np.zeros((base_dataset.timesteps_per_trace,), dtype=np.float32) for key in targets}
