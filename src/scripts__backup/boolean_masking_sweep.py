...
for seed in seeds:
    data_module = datasets.load(
        ...
        dataset_kwargs = {
            ...
            'shuffle_locs': 1,
            'max_no_ops': 0,
            'infinite_dataset': True, # we can deal with overfitting issues using the 'real' datasets
            'data_var': 1.0,
            'residual_var': 1.0,
            'lpf_beta': 0.9
        }
    ...
    for lambda_val in lambda_vals:
        logging_dir = os.path.join(get_trial_dir(), f'boolean_masking__lambda={lambda_val}__seed={seed}')
        ...
        leaking_timesteps = np.concatenate([data_module.train_dataset.leaking_mask_cycles, data_module.train_dataset.leaking_masked_subbytes_cycles])
        ...
        fig.tight_layout()
        fig.savefig(os.path.join(logging_dir, f'boolean_masking__lambda={lambda_val}__seed={seed}.pdf'))
            
        fig, ax = plt.subplots(figsize=(4, 4))
        ax.axvline(data_module.train_dataset.leaking_mask_cycles[0], color='orange', linestyle='dotted', label='Leaking mask')
        ax.axvline(data_module.train_dataset.leaking_masked_subbytes_cycles[0], color='orange', linestyle='dashdot', label='Leaking masked SubBytes')
