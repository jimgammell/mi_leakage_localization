...
Results = OrderedDict([
    ('synthetic_boolean_masking_supervised_htune__preliminary', None),
    ('synthetic_no_ops_supervised_htune__preliminary', None),
    ('synthetic_shuffling_supervised_htune__preliminary', None),
    ('synthetic_unprotected_supervised_htune__preliminary', None)
])
...
for dir, results in Results.items():
    ...
    fig, axes = plt.subplots(3, trial_count, figsize=(2*trial_count, 2*3))
    for trial_idx in range(trial_count):
        axes[0, trial_idx].plot(results['train_loss'][trial_idx, 0, :], results['train_loss'][trial_idx, 1, :], color='red', linestyle='--', rasterized=True)
        axes[0, trial_idx].plot(results['val_loss'][trial_idx, 0, :], results['val_loss'][trial_idx, 1, :], color='red', linestyle='-', rasterized=True)
        axes[1, trial_idx].plot(results['train_rank'][trial_idx, 0, :], results['train_rank'][trial_idx, 1, :], color='red', linestyle='--', rasterized=True)
        axes[1, trial_idx].plot(results['val_rank'][trial_idx, 0, :], results['val_rank'][trial_idx, 1, :], color='red', linestyle='-', rasterized=True)
        rank_over_time = results['rank_over_time'][trial_idx]
        axes[2, trial_idx].fill_between(np.arange(1, rank_over_time.shape[1]+1), np.percentile(rank_over_time, 25, axis=0), np.percentile(rank_over_time, 75, axis=0), color='blue', alpha=0.25, rasterized=True)
        axes[2, trial_idx].plot(np.arange(1, rank_over_time.shape[1]+1), np.median(rank_over_time, axis=0), color='blue', linestyle='-', rasterized=True)
        axes[0, trial_idx].set_title(f'lr={results["learning_rate"][trial_idx]:.1e}')
        axes[2, trial_idx].set_xlabel('Traces seen')
        axes[2, trial_idx].set_ylabel('Correct-key rank')
        axes[1, trial_idx].set_xlabel('Training steps')
        axes[0, trial_idx].set_xlabel('Training steps')
        axes[0, trial_idx].set_ylabel('Loss')
        axes[1, trial_idx].set_ylabel('Mean rank')
        axes[2, trial_idx].set_xscale('log')
    fig.tight_layout()
