...
from utils.metrics.rank import _accumulate_ranks, _process_dataloader_for_rank_accumulation
...
lambdas = [1e-2, 1e-1, 1e0, np.log(256)]
if RUN_SWEEPS:
    for lbda in lambdas:
    ...
    for lbda in lambdas:
        ...
        for learning_rate, ep_ax, rot_ax, loss_ax in zip(learning_rates, ep_axes.flatten(), rot_axes.flatten(), loss_axes.flatten()):
            ...
            for x in range(3):
                ...
                rv = _process_dataloader_for_rank_accumulation(training_module, metadata_keys=['plaintext'], constants=None, skip_forward_passes=True)
