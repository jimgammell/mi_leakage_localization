...
class GetNextLambda:
    def __init__(self):
        ...
        self.upper_bound = np.log(16)
    def get_midpoint(self):
        return 0.5*(self.lower_bound + self.upper_bound)
    def update(self, last_obf_weight):
        if last_obf_weight.max() > 0.9:
            self.lower_bound = self.get_midpoint()
        else:
            self.upper_bound = self.get_midpoint()
...
for learning_rate in LEARNING_RATES:
    ...
    plot_training_curves(training_curves, logging_dir, [['train-loss', 'val-loss'], ['train-rank', 'val-rank']])
...
# Train a standard classifier with the optimal learning rate and early stopping based on validation rank
...
if training_curves is None:
    ...
    training_module = SupervisedClassificationModule(
        model_name='sca-cnn',
        optimizer_name='AdamW',
        model_kwargs={'input_shape': (1, data_module.train_dataset.dataset.timesteps_per_trace), 'output_classes': 16},
...
if training_curves is None:
    ...
    training_module = ALLTrainer(
        ...
        classifier_kwargs={'input_shape': (1, data_module.train_dataset.dataset.timesteps_per_trace), 'output_classes': 16},
